<template>
  <div class="h-full p-6" data-simplebar>
    ...
    <div class="h-[100px] w-[100px] rounded-full mx-auto mb-4">
      <img :src="user.avatar" alt="" class="block w-full h-full object-cover rounded-full" />
    </div>
    ...
    <ul class="list-item mt-5 space-y-4 border-b border-slate-100 dark:border-slate-700 pb-5 -mx-6 px-6">
      <li class="flex justify-between text-sm text-slate-600 dark:text-slate-300 leading-[1]">
        ...
        <div class="font-medium">Argentina</div>
      </li>
      <li class="flex justify-between text-sm text-slate-600 dark:text-slate-300 leading-[1]">
        <div class="flex space-x-2 items-start rtl:space-x-reverse">
        ...
        <div class="font-medium">Oct 2022</div>
      </li>
      <li class="flex justify-between text-sm text-slate-600 dark:text-slate-300 leading-[1]">
        <div class="flex space-x-2 items-start rtl:space-x-reverse">
    ...
    <ul class="list-item space-y-3 border-b border-slate-100 dark:border-slate-700 pb-5 -mx-6 px-6 mt-5">
      <li v-for="(slink, sindex) in socials" :key="sindex" class="text-sm text-slate-600 dark:text-slate-300 leading-[1]">
        <router-link to="#" class="flex space-x-2 rtl:space-x-reverse">
          <Icon :icon="slink.icon" class="text-base" />
          <span class="capitalize font-normal text-slate-600 dark:text-slate-300">{{ slink.name }}</span>
        </router-link>
      </li>
    </ul>
  </div>
