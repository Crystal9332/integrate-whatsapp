          ...
          <div class="flex space-x-3 rtl:space-x-reverse">
            ...
            <div class="flex-none">
              <div class="h-10 w-10 rounded-full relative">
                <span :class="store.user.status === 'active'
                    ? 'bg-success-500'
                    : 'bg-secondary-500'
                  "
                  class="status ring-1 ring-white inline-block h-[10px] w-[10px] rounded-full absolute -right-0 top-0"></span>
                <img :src="store.user.avatar" alt="" class="w-full h-full object-cover rounded-full" />
              </div>
            </div>
    ...
    <div class="chat-content parent-height">
      <div class="msgs overflow-y-auto msg-height pt-6 space-y-6" ref="chatheight">
        <div class="block md:px-6 px-4" v-for="(item, i) in getChats" :key="i">
          <div class="flex space-x-2 items-start group rtl:space-x-reverse" v-if="item.sender === 'them'">
            <div class="flex-none">
              <div class="h-8 w-8 rounded-full">
                <img :src="item.img" alt="" class="block w-full h-full object-cover rounded-full" />
              </div>
            </div>
